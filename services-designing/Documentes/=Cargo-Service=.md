...
4. **Подтверждение/Отмена**:    
    - Если от `Delivery Service` приходит сигнал `PAID` (в `Delivery Service` сигнал попадет через Kafka, topic: `payment.payment-status`), товар переводится в статус «`К отгрузке`».
    - Если счет `EXPIRED` или `CANCELLED`, `Delivery Service` дает сигнал и `Cargo Service` снимает бронь и возвращает товар в доступные остатки.
    - Если пользователь производит отмену доставки товара - то происходит все то-же самое, как и в предыдущем п.
...
7. **Контроль консистентности** (Cleanup): Если `Payment Service` по какой-то причине не прислал сигнал об отмене, а время бронирования (из метаданных счета) вышло, `Cargo Service` может иметь внутренний планировщик (`Scheduler`), который сверяет затянувшиеся `RESERVED` статусы и возвращает их в `AVAILABLE`, давая сигнал по REST в `Delivery Service` (API: `HTTP-API-cargo_delivery`).
